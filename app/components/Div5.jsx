...
export default function Div5(props) {
  const [predictValue_15m, setPredictValue15] = useState("0");
  useEffect(() => {
    const socket = io("https://pred15.azurewebsites.net");
    socket.on('connect',function(){
      console.log(socket)
    })
    socket.on("data_15m", function (data) {
      setPredictValue15(parseFloat(data.message.predict[0]).toFixed(2));
    });
    return () => {
      socket.disconnect();
    };
  ...
  return (
    <div className="w-[260px] md:w-3/6 h-[90px] bg-pink-700 rounded-xl">
      ...
      <div className="w-full h-full flex justify-center items-center gap-3">
        <h2 className="md:text-[30px] text-[20px] -mt-10 text-white">{predictValue_15m != 0 ? predictValue_15m : "Wait ..."}</h2>
