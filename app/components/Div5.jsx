...
export default function Div5(props) {
  ...
  useEffect(() => {
    const sse=new EventSource('https://pred15.azurewebsites.net/stream15m');
  ...
  return (
    <div className="w-[260px] md:w-3/6 h-[90px] md:h-[100px]  bg-pink-700 rounded-xl">
      <div className="flex items-center w-full">
      <div className="w-full flex items-center">
        <h1 className="pt-2 pl-5 font-bold md:text-[20px] text-[10px] text-white">Prediction-15m</h1>
        <h4 className='relative top-1 left-[4.8rem] pl-1 md:pl-0 md:left-[11.1rem] md:text-[18px] text-[10px] font-semibold text-stone-50 w-40'>Next-In : {Timeutc != "" ? <span className='w-10 md:text-[18px] text-[10px] font-semibold text-stone-50 bg-black bg-opacity-75 rounded p-[5px] pt-[1.5px] pb-[1.5px]'>{Timeutc}</span> : <span></span>}</h4>
      </div>
      </div>
