...
export default function Div3(props) {
  const [predictValue_3m, setPredictValue3] = useState("0");
  useEffect(() => {
    const socket = io("https://pred3.azurewebsites.net");
    socket.on('connect',function(){
      console.log(socket)
    })
    socket.on("data_3m", function (data) {
      setPredictValue3(parseFloat(data.message.predict[0]).toFixed(2));
    });
    return () => {
      socket.disconnect();
    };
  ...
  return (
    <div className=" w-[260px] md:w-3/6 h-[90px] bg-blue-400 rounded-xl">
      ...
      <div className="w-full h-full flex justify-center items-center gap-3">
        <h2 className="md:text-[30px] text-[20px] -mt-10 text-white">{predictValue_3m != 0 ?predictValue_3m : "Wait ..."}</h2>
